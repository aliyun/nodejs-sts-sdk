...
const StsBaseClient = require('../lib/sts_base');

describe('sts base client should success with ak', function () {
  it('init sts base client constructor', async function () {
    // check config
    let error = '';
    try {
      new StsBaseClient({
    ...
    expect(error).to.be('config.endpoint must be passed in, please see https://help.aliyun.com/document_detail/28756.html to chose one');
    try {
      new StsBaseClient({
        endpoint: 'sts.aliyuncs.com',
        namespace: 'xxxxxx'
  ...
  it('get query correct', async function () {
    const sign1 = await client.__getQuery({}, {});
    expect(typeof sign1.Signature).to.be('string');
    const sign2 = await client.__getQuery({}, { method: 'POST' });
    expect(typeof sign2.Signature).to.be('string');
    expect(sign1 !== sign2).to.be.ok();
  });
});

describe('sts base client should success with sts credential', function () {
  let client2;
  it('init sts base client constructor', function () {
    let error = '';
    try {
      client2 = new StsBaseClient({
        endpoint: 'sts.aliyuncs.com',
        credential: {
          getCredential() {
            return {
...
describe('sts base client should success with bearer token', function () {
  let client2;
  it('init sts base client constructor', function () {
    let error = '';
    try {
