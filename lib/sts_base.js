...
class StsBase {
  constructor(config) {
    ...
    if (!endpoint) {
      throw new Error('config.endpoint must be passed in, please see https://help.aliyun.com/document_detail/28756.html to chose one');
    }
    ...
    // optional config
    this.apiVersion = config.apiVersion || '2015-04-01';
  }
  ...
  async __getQuery(query, request) {
    ...
    query.AccessKeyId = credential.accessKey;
    query.SignatureMethod = 'HMAC-SHA1';
    query.Timestamp = new Date().toISOString();
    query.SignatureVersion = '1.0';
    query.SignatureNonce = uuid();
    ...
    const sign = `${credential.secretKey}&`;
